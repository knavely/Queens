...
__device__ __host__ MBOARD32 LShift(MBOARD32 B,int k) {
  ...
  for(int i = 0; i < 16; ++i) {
    ...
    if(i == 15 && (~((1ULL << (k)) - 1) & B.board[15])){
      t0 = (~((1ULL << (k)) - 1) & B.board[15]) >> 64 - k;
    }
...
__device__ __host__ MBOARD32 RShift(MBOARD32 B,int k) {
  ...
  for(int i = 16-1; i >= 0; --i) {
    ...
    if(i == 0 && (((1ULL << k) - 1) & B.board[0])){
      t16 = (((1ULL << k) - 1) & B.board[0]) << 64 - k;
    }
    B.board[i] = ((B.board[i] >> k) | t);
    t = (T & ((1ULL << (k)) - 1)) << 64-k;	
  }
  B.board[15] = B.board[15] | t16;
  return B;
}
...
__device__ __host__ MBOARD32 getRookMask(MBOARD32 queens) {
  ...
  MBOARD32 pos = queens;
  int i = 0;
  for(MBOARD32 mask = rookRowMask(); i < 32; mask = LShiftRook(mask,1), ++i) {
  ...
  i = 0;
  for(MBOARD32 mask = rookColMask(); i < 32; mask = LShift(mask,1), ++i) {
      if(Positive(And(queens,mask))) {
...
__device__ __host__ MBOARD32 getBishopMask(MBOARD32 queens) {
  ...
  int i = 0;
  for(MBOARD32 mask1 = BISHOP1, mask2 = BISHOP1; i < 32; mask1 = RShiftBishop1(mask1,1), mask2 = LShiftBishop1(mask2,1), ++i) {   
  ...
  i = 0;
  for(MBOARD32 mask1 = BISHOP2, mask2 = BISHOP2; i < 32; mask1 = RShiftBishop1(mask1,1), mask2 = LShiftBishop1(mask2,1), ++i ) {
     if(Positive(And(queens, mask1))) {
...
__global__ void sample(int *mq, MBOARD32 *mxb) {
  ...
  while(*mq < 100) {
    //mb = genWordT((float)id/20000.0,((float)id/1000.0)+1,id);
   ...
   if((whiteQ < blackQ)) {
    ...
    if(*mq > 80 && (whiteQ >= 80 && blackQ >= *mq || whiteQ >= *mq && blackQ >= 80)) {
      int s = 0;
      int sm = 0;
...
int main() {
  ...
  *mq=0;
  ...
  MBOARD32 g = {.board = {1ULL << 5 | 1ULL << 6 | 1ULL << 7 | 1ULL << 8 | 1ULL << 9,0,0,0,0,0,0,0}};
  drawBoard(RShiftBishop1(bishopDiagonal2(),1),g);
  drawBoard(RShiftBishop1(bishopDiagonal1(),1),g);
  drawBoard(LShiftBishop1(bishopDiagonal2(),1),g);
  drawBoard(LShiftBishop1(bishopDiagonal1(),1),g);
  drawBoard(getBishopMask(g),g);
